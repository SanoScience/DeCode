...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if norm == "batch":
            ...
            norm_type: Type[Union[nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d]] = Norm[
                Norm.INSTANCE, spatial_dims]
        elif norm == "group":
        ...
        self.layer1 = self._make_layer(block, block_inplanes[0], layers[0], spatial_dims, shortcut_type, norm=norm,
                                       act=act)
        self.layer2 = self._make_layer(block, block_inplanes[1], layers[1], spatial_dims, shortcut_type, norm=norm,
                                       act=act, stride=2)
        self.layer3 = self._make_layer(block, block_inplanes[2], layers[2], spatial_dims, shortcut_type, norm=norm,
                                       act=act, stride=2)
        self.layer4 = self._make_layer(block, block_inplanes[3], layers[3], spatial_dims, shortcut_type, norm=norm,
                                       act=act, stride=1)  # stride=1 to match unet, org=2
    ...
    def _make_layer(
        ...
        layers = [
            block(
                in_planes=self.in_planes, planes=planes, spatial_dims=spatial_dims, stride=stride,
                downsample=downsample, act=act, norm=norm
            )
        ]
