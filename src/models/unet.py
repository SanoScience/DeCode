...
if __name__ == "__main__":
    from modules import DAFT, DoubleInputSequential, SkipConnection, FiLM, TabAttention
else:
    from .modules import DAFT, DoubleInputSequential, SkipConnection, FiLM, TabAttention
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        def _create_block(
            ...
            if len(channels) > 2:
                ...
                subblock = self._get_bottom_layer(c, channels[1], self.tabular_module)
                upc = c + channels[1]
    ...
    def _get_connection_block(self, down_path: nn.Module, up_path: nn.Module, subblock: nn.Module) -> nn.Module:
        ...
        """
        return DoubleInputSequential(down_path, SkipConnection(subblock), up_path)

    def _get_down_layer(self, in_channels: int, out_channels: int, strides: int, is_top: bool,
                        tabular_module=None) -> nn.Module:
        """
        ...
        if tabular_module is not None:
            mod = DoubleInputSequential(mod, tabular_module)
        else:
            mod = DoubleInputSequential(mod)
        return mod

    def _get_bottom_layer(self, in_channels: int, out_channels: int, tabular_module=None) -> nn.Module:
        """
        ...
        Args:
            ...
            out_channels: number of output channels.
        """
        return self._get_down_layer(in_channels, out_channels, 1, False, tabular_module=tabular_module)
    ...
    def forward(self, x: torch.Tensor, tab: torch.Tensor = None) -> torch.Tensor:
        x = self.model(x, tab)
        return x
...
if __name__ == "__main__":
    ...
    features = 16
    depth = 5
    channels = tuple(2 ** i * features for i in range(0, depth))
    strides = list((depth - 1) * (2,))
    model = UNet(spatial_dims=3,
                 ...
                 act='relu',
                 norm='instance',
                 tabular_module=None) #DAFT(channel_dim=channels[-1], tab_dim=544)) #FiLM(channel_dim=channels[-1])
