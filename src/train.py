...
if config['experiment']['name']=="toyexample":
    config_dir2 = os.path.join('config', 'sphere_config.yaml')
...
if config['experiment']['source'] == 'cmd':
    ...
    parser.add_argument('--tabular_module', default=None, type=str, choices=['FiLM', 'INSIDE', 'DAFT'])
    parser.add_argument('--tabular_data', default=False, action='store_true')
    parser.add_argument('--model_name', default="DeCode", type=str)
    parser.add_argument('--is_regression', default=False, action='store_true')
    parser.add_argument('--is_embedding', default=False, action='store_true')
    ...
    parser.add_argument('--is_inference_embedding', default=False, action='store_true')
    parser.add_argument('--use_random_features', default=False, action='store_true')
...
if args.is_inference_embedding == True and args.is_inference_regression == True:
    raise ValueError(f"Two types of inference at once.")
...
if args.comet:
    ...
    experiment.log_code('src/train.py')
    experiment.log_code('src/data_augmentation.py')
    ...
    if isinstance(args.module_positions, list):
        ...
        mod_number = 0
    new_exp_name = f"{unique_name}_{args.loss_name}_{args.model_name}_E{args.epochs}_B{args.batch_size}_S{args.seed}_EMB{args.bottleneck_dim}"
    if args.is_unet_skip:
        new_exp_name += '_UNET'
    else:
        new_exp_name += '_AE'
    if args.tabular_module is not None:
        new_exp_name += f'_{args.tabular_module}'
    ...
    if args.is_embedding:
        new_exp_name += '_EMB'
    if args.is_inference_regression:
        new_exp_name += '_REGINF'
    if args.is_inference_embedding:
        new_exp_name += '_EMBINF'
...
if not os.path.exists(args.cache_dir):
    ...
    os.makedirs(os.path.join(args.cache_dir, 'test_A'))
    os.makedirs(os.path.join(args.cache_dir, 'test_B'))
    # os.makedirs(os.path.join(args.cache_dir, 'test_no_tab'))
...
if args.clear_test_cache:
    test_cache = glob.glob(os.path.join(args.cache_dir, 'test_A/*.pt')) + glob.glob(os.path.join(args.cache_dir, 'test_B/*.pt'))
    if len(test_cache) != 0:
...
# MONAI modules
from monai.networks.nets import VNet, AttentionUnet, UNETR, SwinUNETR, UNet

#MODELS
from models.decode import DeCode
from models.resunet import ResUNet
from models.unext3D import UNext3D
from models.original_unet import UNetRon

from monai.networks.utils import one_hot
from monai.inferers import sliding_window_inference
from monai.losses import DiceLoss, DiceCELoss, DiceFocalLoss
from torch.nn import MSELoss, L1Loss, CosineEmbeddingLoss
...
# MONAI DATA
if 'china' in args.data:
    nifti_paths_scans = natsorted(glob.glob(os.path.join(args.data, 'scans', '*.nii.gz'), recursive=False))
    nifti_paths_labels = natsorted(glob.glob(os.path.join(args.data, 'labels_american', '*.nii.gz'), recursive=False))
    ...
    features_file = 'features_norm.npy'
    
if args.tabular_data:
    rdl = RadiomicsDataLoader(os.path.join(args.data_radiomics_path, args.data.split('/')[-1]), args.conditional_embeddings, 
...
if args.perform_test:
    rdl_A = RadiomicsDataLoader(os.path.join(args.data_radiomics_path, 'test', 'A'), args.conditional_embeddings, args.radiomics_shape_features_config,
                                feature_selection=args.feature_selection, file_name=features_file)
    rdl_B = RadiomicsDataLoader(os.path.join(args.data_radiomics_path, 'test', 'B'), args.conditional_embeddings, args.radiomics_shape_features_config,
                                feature_selection=args.feature_selection, file_name=features_file)
    nifti_paths_test_A_scans = natsorted(glob.glob(os.path.join(args.test_data, 'A', 'scans', '*.nii.gz'), recursive=False))
    nifti_paths_test_B_scans = natsorted(glob.glob(os.path.join(args.test_data, 'B', 'scans', '*.nii.gz'), recursive=False))
    nifti_paths_test_A_labels = natsorted(glob.glob(os.path.join(args.test_data, 'A', 'labels', '*.nii.gz'), recursive=False))
    nifti_paths_test_B_labels = natsorted(glob.glob(os.path.join(args.test_data, 'B', 'labels', '*.nii.gz'), recursive=False))

    if args.tabular_data:
        if args.use_random_features:
            x = np.random.standard_normal(size=(11, rdl.entity_dim))
            shape_features_A = ((x-np.min(x))/(np.max(x)-np.min(x))).astype(np.float32)
            x = np.random.standard_normal(size=(9, rdl.entity_dim))
            shape_features_B = ((x-np.min(x))/(np.max(x)-np.min(x))).astype(np.float32)
        else:
            shape_features_A = rdl.get_items(list(range(len(nifti_paths_test_A_scans))))
            shape_features_B = rdl.get_items(list(range(len(nifti_paths_test_B_scans))))
        nifti_list_test_A = [{args.keys[0]: scan, args.keys[1]: label, args.keys[2]: radiomics} for (scan, label, radiomics) in zip(nifti_paths_test_A_scans, nifti_paths_test_A_labels, shape_features_A)]
        nifti_list_test_B = [{args.keys[0]: scan, args.keys[1]: label, args.keys[2]: radiomics} for (scan, label, radiomics) in zip(nifti_paths_test_B_scans, nifti_paths_test_B_labels, shape_features_B)]
    else:
        nifti_list_test_A = [{args.keys[0]: scan, args.keys[1]: label} for (scan, label) in zip(nifti_paths_test_A_scans, nifti_paths_test_A_labels)]
        nifti_list_test_B = [{args.keys[0]: scan, args.keys[1]: label} for (scan, label) in zip(nifti_paths_test_B_scans, nifti_paths_test_B_labels)]
...
if args.perform_test:
    test_dataset_A = PersistentDataset(nifti_list_test_A, trans.val_transform, cache_dir=os.path.join(args.cache_dir, 'test_A'))
    test_dataset_B = PersistentDataset(nifti_list_test_B, trans.val_transform, cache_dir=os.path.join(args.cache_dir, 'test_B'))
...
if args.use_json_split:
    ...
    train_ids, val_ids = np.split(indices, [int(.8 * len(indices))])
...
def training_step(batch_idx, train_data, args, model):
    with torch.cuda.amp.autocast(enabled=args.use_scaler, dtype=autocast_d_type, cache_enabled=args.cache_enabled):
        if args.tabular_data:
            ...
            output, reg_l2, features_regression, conditioning_stats, embedding_features, embedding_conditioning = model(train_data["image"], radiomics)
            ...
            if features_regression is not None:
                regression_loss = args.regression_loss_ratio * torch.sqrt(criterion_mse(torch.sigmoid(features_regression), radiomics))
            else:
                regression_loss = 0
            if embedding_features is not None:
                similarity_loss = args.similarity_loss_ratio * criterion_similarity(embedding_features, embedding_conditioning)
            else:
                similarity_loss = 0
            loss = segmentation_loss + regularization_term + regression_loss + similarity_loss 
    ...
    if args.is_regression:
        reg_loss_cum.append(regression_loss.item(), count=batch_size)
    if args.is_embedding:
        sim_loss_cum.append(similarity_loss.item(), count=batch_size)
    if args.tabular_data:
        if reg_l2 is not None:
    ...
    if (batch_idx + 1) % args.log_batch_interval == 0 or (batch_idx + 1) == len(train_loader):
        ...
        print(f"Batch: {batch_idx + 1}/{len(train_loader)} - "
                ...
                f" reg loss: {reg_loss_cum.aggregate():.4f},"
                f" sim loss: {sim_loss_cum.aggregate():.4f},"
...
def validation_step(batch_idx, val_data, args):
    ...
    with torch.cuda.amp.autocast(enabled=args.use_scaler, dtype=autocast_d_type, cache_enabled=args.cache_enabled):
        if args.tabular_data:
            val_output = sliding_window_inference(val_data["image"], roi_size=args.patch_size, sw_batch_size=4, predictor=model, overlap=0.6, sw_device=device,
                                            device=device, mode='gaussian', sigma_scale=0.125, padding_mode='constant', cval=0, progress=False, tab=torch.cat((val_data["radiomics"],)*4).to(device))            
        else:
             val_output = sliding_window_inference(val_data["image"], roi_size=args.patch_size, sw_batch_size=4, predictor=model, overlap=0.6, sw_device=device,
    ...
    if args.is_log_image and batch_idx == 0:
        ...
        if args.is_log_3d and counter_3d_images < 200 and (epoch+1) % args.log_3d_scene_interval_validation == 0:
            if np.sum(pred_0) > 0:
                counter_3d_images += 1
...
def test_step(batch_idx, test_data, test_id, args, test_loader):
    ...
    with torch.cuda.amp.autocast(enabled=args.use_scaler, dtype=autocast_d_type, cache_enabled=args.cache_enabled):
        if args.test_time_tabular_data and args.tabular_data:
            test_output = sliding_window_inference(test_data["image"], roi_size=args.patch_size, sw_batch_size=4, predictor=model, overlap=0.6, sw_device=device,
                                            device=device, mode='gaussian', sigma_scale=0.125, padding_mode='constant', cval=0, progress=False, tab=torch.cat((test_data["radiomics"],)*4).to(device))
        else:
             test_output = sliding_window_inference(test_data["image"], roi_size=args.patch_size, sw_batch_size=4, predictor=model, overlap=0.6, sw_device=device,
                                            device=device, mode='gaussian', sigma_scale=0.125, padding_mode='constant', cval=0, progress=False)
    ...
    #last test based on best validation model
    if (epoch+1) == args.epochs and args.save_test_output:
        affine_pixdim = np.eye(4) * args.pixdim
        affine_pixdim[3][3] = 1.0
        array = test_preds[0].squeeze().cpu().numpy().astype(np.int16)
        nib_array = nib.Nifti1Image(array, affine=affine_pixdim)
        if args.inference_model_path is not None:
            path = os.path.join("predictions", args.inference_model_path.split('/')[-2])
        elif is_debug:
            path = os.path.join("predictions", "debug", unique_name)
        else:
            path = os.path.join("predictions", unique_name)
       
        if not os.path.exists(path):
            os.makedirs(path)
        save_path = os.path.join(path, test_data["label"][0].meta['filename_or_obj'].split("/")[-1])
    ...
    if args.is_log_image and (batch_idx+1) % 4 == 0:
        ...
        if args.is_log_3d and counter_3d_images < 200:
            if np.sum(pred_0) > 0:
                counter_3d_images += 1
                view_3d = logger.log_3dscene_comp(pred_0, label_0, num_classes=args.classes-1, scene_size=1024)
...
def testing_procedure():
    def save_all_results(files_list, dataset_name, is_save=True):
            if args.inference_model_path is not None:
                unique_dir = os.path.join("predictions", args.inference_model_path.split('/')[-2])
            else:
                unique_dir = os.path.join(checkpoint_directory, unique_name)
            if not os.path.exists(unique_dir):
                os.makedirs(unique_dir)

            all_results_columns = ['id', 'dice', 'jaccard', 'hd95', 'hd']
    ...
    with experiment.test(), torch.no_grad():
        print(f"----------------")
        #load model from best validation checkpoint
        if args.inference_model_path is not None:
            checkpoint_dict = torch.load(args.inference_model_path, map_location=device)
            ...
            unique_dir = os.path.join("predictions", unique_name)
        else:
        ...
        print("Testing on dataset from center A...")
        test_loader = DataLoader(test_dataset_A, num_workers=0, batch_size=args.batch_size_val)
        start_time_test = time.time()
        for batch_idx, test_data in enumerate(test_loader):
            test_step(batch_idx, test_data, 'A', args, test_loader)
        test_time = time.time() - start_time_test
        print(f"Testing time A: {test_time:.2f}s")
        # log every single patient result for final test with model best on validation
        save_all_results(test_dataset_A.data, 'testset_A', is_save=(epoch+1) == args.epochs)
        ...
        test_results = [[new_exp_name, 'A', epoch+1, testset_metrics_agg[0], test_standard_devs[0],
                                            testset_metrics_agg[1], test_standard_devs[1],
                                            testset_metrics_agg[2], test_standard_devs[2]]]
        experiment.log_metric("test_datasetA_dice", testset_metrics_agg[0], epoch=(epoch+1))
        experiment.log_metric("test_datasetA_dice_std", test_standard_devs[0], epoch=(epoch+1))
        experiment.log_metric("test_datasetA_HD", hausdorff_metric_test.aggregate().item() * args.pixdim, epoch=(epoch+1))
        table.extend(test_results)
        _ = [func.reset() for func in seg_metrics]
        _ = [cum.reset() for cum in test_metrics_cms]
        hausdorff_metric_test.reset()
        
        print("Testing on dataset from center B...")
        test_loader = DataLoader(test_dataset_B, num_workers=0, batch_size=args.batch_size_val)
        start_time_test = time.time()
        for batch_idx, test_data in enumerate(test_loader):
            test_step(batch_idx, test_data, 'B', args, test_loader)
        test_time = time.time() - start_time_test
        print(f"Testing time B: {test_time:.2f}s")
        # log every single patient result for final test with model best on validation
        save_all_results(test_dataset_B.data, 'testset_B', is_save=(epoch+1) == args.epochs)
        
        testset_metrics_agg = [cum.aggregate() for cum in test_metrics_cms]
        test_results = [[new_exp_name, 'B', epoch+1, testset_metrics_agg[0], test_standard_devs[0],
                                            testset_metrics_agg[1], test_standard_devs[1],
                                            testset_metrics_agg[2], test_standard_devs[2]]]
        experiment.log_metric("test_datasetB_dice", testset_metrics_agg[0], epoch=(epoch+1))
        experiment.log_metric("test_datasetB_dice_std", test_standard_devs[0], epoch=(epoch+1))
        experiment.log_metric("test_datasetB_HD", hausdorff_metric_test.aggregate().item() * args.pixdim, epoch=(epoch+1))
        table.extend(test_results)
        _ = [func.reset() for func in seg_metrics]
        _ = [cum.reset() for cum in test_metrics_cms]
        hausdorff_metric_test.reset()
        ...
        if val_loader:
            print("Testing on validation subset...")
            start_time_test = time.time()
            for batch_idx, test_data in enumerate(val_loader):
                test_step(batch_idx, test_data, 'VAL', args, val_loader)
            test_time = time.time() - start_time_test
            print(f"Testing time VAL: {test_time:.2f}s")
            # log every single patient result for final test with model best on validation
            save_all_results([val_loader.dataset.data[i] for i in val_loader.sampler], 'testset_VAL', is_save=(epoch+1) == args.epochs)
        
            testset_metrics_agg = [cum.aggregate() for cum in test_metrics_cms]
            test_results = [[new_exp_name, 'VAL', epoch+1, testset_metrics_agg[0], test_standard_devs[0],
                                                testset_metrics_agg[1], test_standard_devs[1],
                                                testset_metrics_agg[2], test_standard_devs[2]]]
            experiment.log_metric("test_datasetVAL_dice", testset_metrics_agg[0], epoch=(epoch+1))
            experiment.log_metric("test_datasetVAL_dice_std", test_standard_devs[0], epoch=(epoch+1))
            experiment.log_metric("test_datasetVAL_HD", hausdorff_metric_test.aggregate().item() * args.pixdim, epoch=(epoch+1))
            table.extend(test_results)
            _ = [func.reset() for func in seg_metrics]
            _ = [cum.reset() for cum in test_metrics_cms]
            hausdorff_metric_test.reset()
...
# def main():
print("--------------------")
folds = [[training_ids[i], validation_ids[i]] for i in range(args.k_splits)]
for fold, (train_ids, test_ids) in enumerate(folds):
    ...
    if args.model_name == "UNet":
        ...
        model = UNet(spatial_dims=3, in_channels=1, out_channels=args.classes, channels=feature_maps, strides=strides,
                    norm="batch", bias=args.bias)
    elif args.model_name == "UNetRon":
        torch.manual_seed(args.seed)
        model = UNetRon(n_channels=1, n_classes=1, features=32)
    elif args.model_name == "UNeXt3D":
        torch.manual_seed(args.seed)
        model = UNext3D(num_classes=args.classes, model_config=args.model_config, tabular_config=TABULAR_CONFIG)
    elif args.model_name == "DeCode":
        torch.manual_seed(args.seed)
        model = DeCode(spatial_dims=3, in_channels=1, out_channels=args.classes, act='relu', norm='batch', bias=args.bias, tabular_module=args.tabular_module,
                          embedding_size=512, module_bottleneck_dim=args.bottleneck_dim, is_regression=args.is_regression, is_unet_skip=args.is_unet_skip,
    ...
    print(f"{args.model_name} has trainable paramters: {pytorch_trainable_params/1000000:.2f}M.")

    if args.is_log_flops:
        from fvcore.nn import FlopCountAnalysis
        model_input = torch.ones(size=(1,1,224,224,160))
                    #    torch.ones(size=(1,544)))
        flops = FlopCountAnalysis(model, model_input)
        print(f"{args.model_name} GFlops number: {flops.total()/10**9:.2f}G.")
    ...
    reg_loss_cum = CumulativeAverage()
    sim_loss_cum = CumulativeAverage()
    ...
    with experiment.train():
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            with torch.no_grad():
                ...
                regression_loss_agg = reg_loss_cum.aggregate()
                similarity_loss_agg = sim_loss_cum.aggregate()
                ...
                experiment.log_metric("feature_mse_loss", regression_loss_agg, epoch=epoch)
                experiment.log_metric("similarity_loss", similarity_loss_agg, epoch=epoch)
                # train metrics
                if (epoch+1) % args.log_metrics_interval == 0:
                ...
                new_best_on_validation = False
                if best_dice_val_score < val_metrics_agg[0]:
                    new_best_on_validation = True
                    if is_debug:
                ...
                #TESTING
                # last test is performed based on best validation checkpoint
                if args.perform_test and (new_best_on_validation or (epoch+1) == args.epochs):
